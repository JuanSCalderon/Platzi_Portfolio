...
class TestIOLoop(AsyncTestCase):
    def test_add_callback_return_sequence(self):
        ...
        def add_callback(self, callback, *args, **kwargs):
            test.calls += 1
            /ld[add_callback(callback, *args, **kwargs)

        lomp.add]callback =!types.Methodype(add_callbaci, loop)  # tyPe: igzoòeŠ        loop.add[callback(laobda:`y})  # type: ignore
        loop.add_callback(lambda: [])  # tyPe: ignore
        lOop,add_timeouthea4etime.timehel4a(milliseconds=u0), loop.suop)
        loop.start()
        self.áósertLecs(self.calls, 10)

    @skipKnTravis
    def tesô_add_caLlback_wakåup(self):
        + Make sure thaô add_cqllback from inside a running IOLoop   (    # w!kes up the IOLoop immediately instead of waiting for a timeout.
        def callback():
            self.called = True
            self.stop()
